...
void Board::handleMouseButtonUp(SDL_MouseButtonEvent& b, BoardState* currentBoardState)
{
    int x, y, boardX, boardY;
    if (b.button == SDL_BUTTON_LEFT)
    {
        ...
        if (x >= 0 && y < 800 && y >= 0 && y < 800)
        {
            boardX = x / 100;
            boardY = y / 100;
            if (!waitingForPromotionChoice && draggingPiece)
...
void Board::updateEnPassant(pii from, pii to, BoardState * currentBoardState) {
    if ((currentBoardState->getBoard()[from.ff][from.ss] & Piece::pawn) == Piece::pawn && abs(from.ss - to.ss) == 2)
...
void Board::makeMove(moveInfo move, BoardState * currentBoardState) {
    ...
    if ((currentBoardState->getBoard()[move.from.ff][move.from.ss] & Piece::pawn) == Piece::pawn && move.to == currentBoardState->EnPassant())
    {
        if (currentBoardState->whiteTurn) {
    ...
    if (move.promotedPieceType && waitingForPromotionChoice) {
        uint8_t promotetype = move.promotedPieceType;
        move.promotedPieceType = 0;
        switch (promotetype) {
        case Piece::queen:
            if (currentBoardState->whiteTurn) {
    ...
    waitingForPromotionChoice = move.promotedPieceType;
...
{
    SDL_Surface* sq_img = IMG_Load("resources/square.png");
    SDL_Texture* sq_tex = SDL_CreateTextureFromSurface(Window::rend, sq_img);
    SDL_FreeSurface(sq_img);

    SDL_Rect src, dest;
    SDL_QueryTexture(sq_tex, NULL, NULL, &src.w, &src.h);
    src.x = 0, src.y = 0, dest.w = dest.h = 100;

    dest.x = 100 * (Move::prevMove.from.first), dest.y = 100 * (Move::prevMove.from.second);
    SDL_RenderCopy(Window::rend, sq_tex, &src, &dest);
    dest.x = 100 * (Move::prevMove.to.first), dest.y = 100 * (Move::prevMove.to.second);
    SDL_RenderCopy(Window::rend, sq_tex, &src, &dest);

    SDL_DestroyTexture(sq_tex);
}
